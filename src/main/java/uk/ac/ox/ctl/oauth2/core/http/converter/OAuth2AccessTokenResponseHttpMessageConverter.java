...
/**
 * A {@link HttpMessageConverter} for an {@link OAuth2AccessTokenResponse OAuth 2.0 Access Token
 * Response}. This file is needed because the Canvas Access Token response includes a parameter with
 * an object value and this was causing the JSON deserialisation to blow up. This class copes with
 * it.
 *
 * @author Joe Grandja
...
public class OAuth2AccessTokenResponseHttpMessageConverter
   ...
   *
   * @param tokenResponseConverter the {@link Converter} used for converting to an {@link
   ...
   *
   * @param tokenResponseParametersConverter the {@link Converter} used for converting to a {@code
   *     Map} representation of the Access Token Response parameters
   */
  public final void setTokenResponseParametersConverter(
    ...
    Assert.notNull(
        tokenResponseParametersConverter, "tokenResponseParametersConverter cannot be null");
  ...
  private static class OAuth2AccessTokenResponseConverter
    ...
    public OAuth2AccessTokenResponse convert(Map<String, Object> tokenResponseParameters) {
      Object o = tokenResponseParameters.get(OAuth2ParameterNames.ACCESS_TOKEN);
      // When we have a scope=/user/authinfo we don't get an access token back, but we need a value, so use "null"
      String accessToken =  (o != null) ? o.toString() : "null";
      ...
      if (tokenResponseParameters.containsKey(OAuth2ParameterNames.EXPIRES_IN)) {
        try {
          expiresIn =
              Long.valueOf(tokenResponseParameters.get(OAuth2ParameterNames.EXPIRES_IN).toString());
      ...
      Object o1 = tokenResponseParameters.get(OAuth2ParameterNames.REFRESH_TOKEN);
      String refreshToken = (o1 != null)? o1.toString() : null;
  ...
  /**
   * A {@link Converter} that converts the provided {@link OAuth2AccessTokenResponse} to a {@code
